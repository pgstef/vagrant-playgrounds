...
```bash
$ sudo -iu postgres /usr/lib/postgresql/16/bin/pg_checksums \
    -D /var/lib/postgresql/16/main --enable --progress
...
sudo systemctl disable --now postgresql@17-main.service
```
...
Run `sudo pg_createcluster 17 main` if the cluster haven't been created when the packages were installed.
...
```bash
cat<<EOF | sudo tee "/var/lib/postgresql/17/main/postgresql.auto.conf"
listen_addresses = '*'
archive_mode = on
...
```bash
sudo -iu postgres sh -c 'echo "host replication replicator postgres0 scram-sha-256" >> /etc/postgresql/17/main/pg_hba.conf'
sudo -iu postgres sh -c 'echo "host replication replicator postgres1 scram-sha-256" >> /etc/postgresql/17/main/pg_hba.conf'
sudo -iu postgres sh -c 'echo "host all all bouncer scram-sha-256" >> /etc/postgresql/17/main/pg_hba.conf'
sudo sed -i 's/5433/5432/g' /etc/postgresql/17/main/postgresql.conf
```
...
```bash
sudo sed -i 's/\/var\/lib\/postgresql\/16\/main/\/var\/lib\/postgresql\/17\/main/g' /etc/pgbackrest.conf
...
```bash
sudo -iu postgres /usr/lib/postgresql/17/bin/vacuumdb --all --analyze-in-stages
```
...
sudo -iu postgres cat /var/lib/postgresql/17/main/postgresql.auto.conf
sudo systemctl start postgresql@17-main.service
sudo systemctl enable postgresql@17-main.service
```
...
```bash
$ sudo -iu postgres /usr/lib/postgresql/17/bin/pg_controldata \
...
```bash
sudo -iu postgres sh -c 'echo "primary_conninfo = '\''host=postgres0 port=5432 user=replicator'\''" >> /var/lib/postgresql/17/main/postgresql.auto.conf'
sudo -iu postgres touch /var/lib/postgresql/17/main/standby.signal
sudo systemctl start postgresql@17-main.service
```
